...
def main():
    ...
    if args.label_token_mode == "AMuLaP":
        ...
        for i in range(tokenizer.vocab_size):
            #### EDIT #####
            # token_id_remove = [8,42,134,1635,2213,5804,9234,13404,23411]
            # if (len(str(tokenizer.decode(i)).replace(' ','')) >=2) and (i not in token_id_remove):
            ###############
    ...
    ##### EDIT #####
    print('')
    print(model.label_token_list)
    for label_values in model.label_token_list.values():
        print('')
        print([tokenizer.decode(x) for x in label_values.tolist()])
        
    test_dataset = processed_datasets["test"]
    test_dataloader = DataLoader(test_dataset, collate_fn=data_collator, batch_size=4)
    pred_list,act_list = [],[]
    for step, batch in enumerate(test_dataloader):
        outputs = model(batch["input_ids"].cuda(), batch["attention_mask"].cuda(), batch["mask_pos"].cuda())
        predictions = outputs[0].argmax(dim=-1)
        pred_list.append(accelerator.gather(predictions))
        act_list.append(accelerator.gather(batch["labels"]))
    pred_list = [item.cpu().tolist() for sublist in pred_list for item in sublist]
    act_list = [item.cpu().tolist() for sublist in act_list for item in sublist]  

    from sklearn.metrics import classification_report
    print('')
    print(classification_report(act_list, pred_list, digits=4))
    ########################
